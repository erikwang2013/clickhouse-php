...
| `select([...])` / `selectRaw($expr)` | Столбцы SELECT. Имена столбцов в `select()` оборачиваются в бэктики (так можно использовать зарезервированные слова вроде `` `order` ``), в алиасе `id as uid` бэктиками обёрнута каждая часть отдельно; для функций и подзапросов используйте `selectRaw()` или `Expression` |
| `where($col, $op, $val)` | Условие (при 2 аргументах `$op` по умолчанию `=`). При значении `null` автоматически превращается в `IS NULL` / `IS NOT NULL` |
...
| `whereRaw($sql)` | Сырой WHERE (не передавайте пользовательский ввод) |
| `prewhere($col, $op, $val)` | PREWHERE, идёт перед WHERE (самое эффективное отсечение сканирования в ClickHouse) |
| `orderBy($col, $dir)` | Сортировка (по умолчанию ASC) |
| `groupBy(...$cols)` | Группировка |
| `having($col, $op, $val)` / `havingRaw($sql)` | HAVING, идёт после GROUP BY. В `having()` имя столбца трактуется как идентификатор и экранируется, поэтому для агрегатных условий (например, `count() > 100`) используйте `havingRaw()` или `new Expression('count()')` |
| `limit($n)` / `offset($n)` | Пагинация |
| `final()` | Дедупликация при чтении (ReplacingMergeTree и подобные) |
| `sample($ratio)` | Сэмплирование SAMPLE, например `sample(0.1)` |
| `settings([...])` | SETTINGS на уровне запроса, например `settings(['max_execution_time' => 30])` |
| `count()` / `sum($col)` / `avg($col)` / `min($col)` / `max($col)` | Агрегация |
| `insert($data)` | Вставка (одна строка или пакет). В одной партии у всех строк должен быть одинаковый набор столбцов: если столбцов не хватает или их больше, будет ошибка (иначе значения записались бы не на свои места) |
| `delete()` | Удаление (компилируется в `ALTER TABLE ... DELETE`, **обязательно с WHERE**, иначе исключение) |
| `get()` | Выполнить запрос, вернуть Result |
| `first()` | Вернуть первую запись |
| `toSql()` | Получить сгенерированный SQL |

### Большие результаты и потоковое чтение

`get()` разбирает весь результат в PHP-массив, поэтому памяти уходит примерно в 7 раз больше объёма ответа (замер на узкой таблице из 5 столбцов: 93 B на строку в ответе → 677 B на строку после разбора, около 73 MB на 100 000 строк). При больших объёмах используйте `stream()` — построчное потребление, память не зависит от размера результата:

```php
use Erikwang2013\ClickHouse\Client\StreamingClientInterface;

$client = ClickHouse::client();           // нужен низкоуровневый клиент — берите его (connection() возвращает конструктор)
...
// Запросы со своим FORMAT (CSV/TSV и т. п.) — через raw(), тело ответа как есть
$csv = $client->raw('SELECT * FROM logs FORMAT CSV');
```

В режиме пула `stream()` тоже работает: соединение возвращается в пул, когда генератор дочитан до конца (или уничтожен досрочным `break`).

### Входные точки для сырого SQL

Следующие точки входа — каналы сырого SQL со **вставкой как есть**: передать туда пользовательский ввод равносильно тому, чтобы отдать базу данных в чужие руки: `selectRaw()`, `whereRaw()`, `havingRaw()`, `new Expression($sql)`, значения `Blueprint::settings()`, а также строки типов столбцов вида `$table->string('col')` (`array($name, $type)`). Сами идентификаторы и значения экранируются (имена столбцов — бэктиками, значения — по типу), но с фрагментами сырого SQL не делается ничего.

`Expression` можно передавать в `select()` (положив в массив), `where()`, `prewhere()`, `having()`, `orderBy()`, `groupBy()` — для выражений вроде `rand()`, `toStartOfHour(ts)`.
...
О пуле соединений: конфигурация `pool` действует только при **наличии доступного корутинного канала** (Swoole / Swow / Workerman); в синхронных средах вроде FPM она игнорируется и соединение идёт напрямую, лимит параллелизма при этом не появится. Кроме того, HTTP-драйвер работает через синхронный Guzzle, так что реальная выгода пула — «ограничение числа одновременных соединений + переиспользование объектов соединений», и **неблокирующим он сам по себе не станет** — для настоящей неблокируемости нужно самому включить curl-хук Swoole (`Swoole\Runtime::enableCoroutine(SWOOLE_HOOK_NATIVE_CURL)`, его нет в `SWOOLE_HOOK_ALL`) или подключить CoroutineHandler из hyperf/guzzle. Драйвер пула можно задать явно через `pool.driver`: `swoole|swow|workerman|none`.
...
В чистом PHP эти переменные читают `ClickHouse::bootstrap()` / `Manager::fromEnv()`; конфигурационные файлы всех четырёх фреймворков используют тот же набор имён переменных (включая `CLICKHOUSE_HTTPS`).
